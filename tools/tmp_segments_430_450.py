...
def body_html(lang,slug):
 ...
 if slug=='changelog':return f'<h2>2026-06-12</h2><p>{html.escape(S.NAMES[lang])}: 77 · 34 · HTML · PDF · DOCX · MD · TXT</p>'
...
def write_page(lang,slug):
 ...
 image=S.thumbnail(slug)
 doc=f'<!doctype html><html lang="{lang}" dir="ltr">{S.head(lang,title,slug,"../../")}<body class="public-page public-page-{lang} design-prompt-theme section-{slug}">{S.header(lang,slug,"../../")}<main class="site-main" id="main"><nav class="breadcrumbs"><a href="../../{lang}.html">{S.NAMES[lang]}</a><span>›</span><span>{html.escape(title)}</span></nav>{S.note(lang)}<section class="page-title media-page-title"><div class="page-title-head"><img class="page-title-mark" src="../../figures/{image}" alt="{html.escape(title)}"><h1>{html.escape(title)}</h1></div></section><section class="reader-layout"><article class="reader-card long-read tone-card bpi-localized-content">{body_html(lang,slug)}</article></section><section class="reader-card"><div class="bpi-localized-link-grid">{siblings}</div></section></main><footer class="site-footer"><p>Between Potential and Ideal — {S.NAMES[lang]} Public Beta</p></footer></body></html>'
